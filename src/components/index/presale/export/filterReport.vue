      ...
      <v-card-text>
        <div>
          <v-flex xs12 sm12 md12 lg12>
            ...
            <div v-if="this.userData.position === 'PRESALES' || this.userData.position === 'TEAMLEAD' ">
              <v-layout row wrap spacing>
                <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Type report : </v-subheader></v-flex>
                <v-flex xs6 sm6 md3 lg3><v-select :items="typeReportpre" v-model="valueType" item-text="text" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
              </v-layout>
            </div>
            <div v-if="this.userData.position === 'GROUPLEAD'  ">
              <v-layout row wrap spacing>
                <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Type report : </v-subheader></v-flex>
                <v-flex xs6 sm6 md3 lg3><v-select :items="typeReportgroup" v-model="valueType" item-text="text" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
              </v-layout>
            </div>
          ...
          <div v-if="valueType !== null ">
            ...
            <div v-if=" valueType.type === 'Weekow' || valueType.type === 'Week'">
              <v-flex xs12 sm12 md12 lg12>
                <v-layout row wrap spacing>
                  <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Month From : </v-subheader></v-flex>
                  <v-flex xs6 sm6 md2 lg2><v-select :items="month" v-model="valueMonth" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
                  <v-flex xs5 sm5 md2 lg2 pl-5><v-subheader class="fontSize">Year no. : </v-subheader></v-flex>
                  <v-flex xs6 sm6 md2 lg2><v-select :items="yearAll" v-model="valueYear" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
                  </v-layout>
              </v-flex>
            </div>
            ...
            <!-- <div v-if=" valueType.type === 'Weekow' || valueType.type === 'Quarter' || valueType.type === 'Week'">
            <v-flex xs12 sm12 md12 lg12>
              <v-layout row wrap spacing>
                <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Year no. : </v-subheader></v-flex>
                <v-flex xs6 sm6 md3 lg2><v-select :items="yearAll" v-model="valueYear" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
  ...
  methods: {
    downloadReport () {
      if (this.valueType.type === 'Week' ) {
        if (this.valueMonth === null || this.valueMonth === null) {
          this.$swal('กรุณาเลือกเดือน', '', 'error')
        } else if (this.valueYear === null ){
          ...
          if(this.valueMonth > this.valueMonth1 && this.valueYear >= this.valueYearTo){
            ...
            // var source = Encode.encode(JSON.stringify({ 'valueYear': this.valueYear, 'valueType': this.valueType.type, 'valueMonth': this.date, 'valueMonth1':this.date2}))
        ...
        var source = Encode.encode(JSON.stringify({emp_no: this.userData.emp_no,'valueYear': this.valueYear, 'valueType': this.valueType.type}))
        this.axios.post(process.env.API + '/generateExcelWeekBusiness', {source: source}).then((response) => {
          var result = JSON.parse(Decode.decode(response.data))
          console.log(result)
          if (result.status === 'success') {
      ...
      } else if (this.valueType.type === 'Summary Growth') {
