<template lang="html">
  <div class="fontSizeNavbar">
    <v-navigation-drawer fixed :clipped="$vuetify.breakpoint.mdAndUp" app v-model="drawer">
      <v-list>
        <template v-for="item in itemsSale">
          <v-list-tile v-if="item.subNormalItems.length === 0 && item.subVPItems.length === 0 && (item.permission === 'ALL' || userData.position === item.permission)" @click="toPage(item.url)" :key="item.text">
            <v-list-tile-action>
          ...
          <v-list-group v-else-if="item.permission === 'ALL' || userData.position === item.permission" :prepend-icon="item.icon" v-model="item.active" no-action>
            <v-list-tile slot="activator">
              <v-list-tile-content>
                <v-list-tile-title>{{ item.text }}</v-list-tile-title>
              </v-list-tile-content>
            </v-list-tile>
            <div v-if="userData.position === 'SALES'">
              <v-list-tile v-for="(sub, i) in item.subNormalItems" :key="i" @click="toPage(sub.url)">
                <v-list-tile-title>{{sub.text}}</v-list-tile-title>
              </v-list-tile>
            </div>
            <div v-else-if="userData.position === 'TEAMLEAD' ">
              <v-list-tile v-for="(sub, i) in item.subVPItems" :key="i" @click="toPage(sub.url)">
                <v-list-tile-title>{{sub.text}}</v-list-tile-title>
              </v-list-tile>
            </div>
            <div v-else-if=" userData.position === 'GROUPLEAD'">
              <v-list-tile v-for="(sub, i) in item.subVPGL" :key="i" @click="toPage(sub.url)">
                <v-list-tile-title>{{sub.text}}</v-list-tile-title>
              </v-list-tile>
            </div>
          </v-list-group>
    ...
    <v-menu offset-y>
      <v-btn slot="activator" flat class="arrow-drop-down">
      ...
      itemsSale: [
        {
          ...
          permission: 'ALL',
          active: true,
          subVPGL: [{text: 'Status', url: 'approve'}],
          subVPItems: [{text: 'Request', url: 'request'}, {text: 'Status', url: 'approve'}],
          subNormalItems: [{text: 'Request', url: 'request'}, {text: 'Status', url: 'approve'}]
        },
        {
          icon: 'add_box',
