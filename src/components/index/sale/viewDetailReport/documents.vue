    ...
    <v-layout row wrap>
      ...
      <v-flex xs10 offset-xs1 class="borderRadius bgBody">
        <v-layout row wrap class="m-b-2">
          ...
          <v-flex xs12 class="m-t-2">
            <v-layout row wrap class="centerHeightText">
              <v-flex xs2 sm2 md2 offset-xs1>Presale Order Status:</v-flex>
              <v-flex xs2>
                <div>
                  <v-chip>
                    <v-avatar :color="colorOrder" size="36">{{order}}</v-avatar>
...
export default {
  ...
  methods: {
    setValue (source) {
      this.date_proceed = this.formatDate(source.date_proceed)
      this.namesotype = source.namesotype
      ...
      this.gradename = source.gradename
      this.resultname = source.resultname
      this.resultremark = source.resultremark
    },
    formatDate (date) {
      if (!date) return null
      moment.locale('en')
