    ...
    <v-layout row wrap>
      ...
      <v-flex xs10 offset-xs1 class="borderRadius bgBody">
        ...
        <v-layout  row wrap class="m-b-2">
          ...
          <v-flex xs4>{{resultname}}</v-flex>
          <v-flex xs1>Result Remark:</v-flex>
          <v-flex xs3 pl-5>{{resultremark}}</v-flex>
        </v-layout>



        <v-layout  row wrap class="m-b-2">
          <v-flex xs2 offset-xs1>Presales Status:</v-flex>
          <v-flex xs4>{{status_presale}}</v-flex>
          <v-flex xs1>Presales Remark:</v-flex>
          <v-flex xs3 pl-5 >{{remark_presale}}</v-flex>
        </v-layout>
...
export default {
  ...
  methods: {
    setValue (source) {
      ...
      this.status_presale = source[0].status_presale
      this.remark_presale = source[0].remark_presale
      console.log(this.status_presale )
    },
    formatDate (date) {
      if (!date) return null
      moment.locale('en')
