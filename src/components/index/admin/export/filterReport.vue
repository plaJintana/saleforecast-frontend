      ...
      <v-card-text>
        <div >
          <v-flex xs12 sm12 md12 lg12>
            <div v-if="this.userData.type_userAdmin === 'SALE' ||  this.userData.type_userAdmin === 'PRESALE'">
              <v-layout row wrap spacing>
                <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Type report : </v-subheader></v-flex>
                <v-flex xs6 sm6 md3 lg3><v-select :items="typeReport" v-model="valueType" item-text="text" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
              </v-layout>
            </div>
            <div v-if="this.userData.type_userAdmin === 'BUSINESS'">
              <v-layout row wrap spacing>
                <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Type report : </v-subheader></v-flex>
                <v-flex xs6 sm6 md3 lg3><v-select :items="typeReportBD" v-model="valueType" item-text="text" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
              </v-layout>
            </div>
          </v-flex>
          <div v-if="valueType !== null ">
            <div v-if=" valueType.type === 'Weekow'">
              <v-flex xs12 sm12 md12 lg12>
                <v-layout row wrap spacing>
                  ...
                  <v-flex xs6 sm6 md3 lg2><v-select :items="month" v-model="valueMonth" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" required autocomplete></v-select></v-flex>
                </v-layout>
              </v-flex>
            </div>
            ...
            <div v-if=" valueType.type === 'Week'">
              ...
              <v-flex xs12 sm12 md12 lg12>
                <v-layout row wrap spacing>
                  <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Date To : </v-subheader></v-flex>
                  <v-flex xs6 sm6 md3 lg2>
                    <v-menu
                    ref="menu2"
                    :close-on-content-click="false"
        ...
        <div v-if=" valueType.type === 'Weekow' || valueType.type === 'Quarter'">
          <v-flex xs12 sm12 md12 lg12>
            <v-layout row wrap spacing>
              <v-flex xs5 sm5 md2 lg2><v-subheader class="fontSize">Year no. : </v-subheader></v-flex>
              <v-flex xs6 sm6 md3 lg2><v-select :items="yearAll" v-model="valueYear" :rules="[v => !!v || 'กรุณาเลือกข้อมูล']" autocomplete required></v-select></v-flex>
  ...
  methods: {
    downloadReport () {
      ...
      if (this.valueType.type === 'Week' ){
        ...
        // this.dateto2 = moment().week(valueWeek2).weekday(8).set('year', this.valueYear).set( 'hour', 11).set( 'minutes', 0).set( 'seconds', 0).format('YYYY-MM-DD HH:mm:ss')
        // // if week === 1
        // if (this.valueWeek === 1) {
        //   this.checkmonth = moment().week(this.valueWeek).weekday(1).set('year', this.valueYear).set( 'hour', 11).set( 'minutes', 1).set( 'seconds', 0).format('M')
        //   this.dateto1 = moment().week(52).weekday(8).set('year', this.valueYear).set('year', this.valueYear).set('date', 31).set( 'hour', 11).set( 'minutes', 0).set( 'seconds', 0).format('YYYY-MM-DD HH:mm:ss')
        //   this.datefrom1 = moment().week(48).set('year', valueYear1).set('date', 1).set('months', 12).set( 'hour', 11).set( 'minutes', 0).set( 'seconds', 0).format('YYYY-MM-DD HH:mm:ss')
        //   // console.log(this.datefrom1)
        //
        // }else{
        //   this.checkmonth = moment().week(this.valueWeek).weekday(1).set('year', this.valueYear).set( 'hour', 11).set( 'minutes', 1).set( 'seconds', 0).format('M')
        //   this.datefrom1 = moment().week(this.valueWeek).weekday(1).set('year', this.valueYear).set( 'hour', 11).set( 'minutes', 1).set( 'seconds', 0).format('YYYY-MM-DD HH:mm:ss')
        //   this.dateto1 = moment().week(52).weekday(7).set('year', this.valueYear).set('date', 31).set( 'hour', 11).set( 'minutes', 0).set( 'seconds', 0).format('YYYY-MM-DD HH:mm:ss')
        ...
        if (this.valueMonth > this.valueMonth1) {
          ...
          var source = Encode.encode(JSON.stringify({'userData': this.userData, 'valueYear': this.valueYear, 'valueType': this.valueType.type, 'valueMonth': this.date, 'valueMonth1':this.date2}))
          this.axios.post(process.env.API + '/generateExcelWeekBusinessadmin', {source: source}).then((response) => {
        ...
        if (this.valueWeek === null || this.valueYear === null || this.valueMonth === null || this.valueMonth1 === null){
          ...
          // console.log(this.valueQuarter)
          // console.log(valueWeek3)1234567890
          // console.log(valueWeek4)
        ...
        var source = Encode.encode(JSON.stringify({year: this.valueYear.toString(), position: this.userData.position, emp_no: this.userData.emp_no, groupline: this.userData.groupline_id, group_id: this.userData.group_id, date_from: this.datefrom, date_to: this.dateto}))
