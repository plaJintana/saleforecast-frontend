  ...
  <div class="p-t-1-5">
    <v-container>
      <div class="">
        ...
        <v-flex xs3 md3 lg3 class="headerFont"><h1>จัดการข้อมูล User</h1></v-flex>
        <v-layout row wrap>
          <!-- style="padding-bottom: 12px;padd -->
          <v-flex xs12 sm8 class="p-t-1-5 p-b-1-5"><v-btn color="info" @click="openRegister()">เพิ่ม User</v-btn></v-flex>
        ...
        <v-data-table :headers="header" :items="filterBySearchBox" v-bind:pagination.sync="pagination" class="elevation-1">
...
export default {
  ...
  methods: {
    getUser () {
      this.axios.post(process.env.API + '/getAllUser').then((response) => {
    ...
    openRegister () {
      this.$refs.register.openDialog('manageUser')
    },
    refresh () {
      this.getUser()
    },
